...
function FadeIn({ children, delay = 0 }: { children: ReactNode; delay?: number }) {
  ...
  return (
    <div ref={ref} style={{ opacity: 0, transform: "translateY(32px)", transition: `opacity 0.7s ease ${delay}ms, transform 0.7s ease ${delay}ms` }}>
      {children}
    </div>
  );
}
...
export default function Home() {
  return (
    <div style={{ paddingTop: 76 }}>
      ...
      {/* ── Hero ── */}
      <section style={{ minHeight: "85vh", display: "flex", alignItems: "center", padding: "6rem 2.5rem", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, backgroundImage: "url('/moore-school.jpg')", backgroundSize: "cover", backgroundPosition: "center 40%", filter: "brightness(0.35)", zIndex: 0 }} />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(0,0,0,0) 50%, #FFFFFF 100%)", zIndex: 1 }} />
        <div style={{ maxWidth: 1200, margin: "0 auto", width: "100%", position: "relative", zIndex: 2 }}>
          <p style={{ fontFamily: "system-ui, sans-serif", fontSize: "0.78rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.6)", marginBottom: "1.5rem" }}>Overview</p>
          <h1 style={{ fontFamily: "Palatino Linotype, Palatino, Georgia, serif", fontSize: "clamp(2.8rem, 6vw, 5rem)", fontWeight: 400, lineHeight: 1.12, letterSpacing: "-0.01em", color: "#FFFFFF", maxWidth: 780, marginBottom: "2.5rem" }}>
            A student-managed investment fund
          </h1>
          <p style={{ fontFamily: "system-ui, -apple-system, sans-serif", fontSize: "1.15rem", color: "rgba(255,255,255,0.75)", lineHeight: 1.8, maxWidth: 520 }}>
      ...
      <section style={{ padding: "7rem 2.5rem", background: "#FFFFFF" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8rem", alignItems: "center" }}>
          <FadeIn>
            ...
            <h2 style={{ fontFamily: "Palatino Linotype, Palatino, Georgia, serif", fontSize: "2.2rem", fontWeight: 400, lineHeight: 1.25, color: "#1A1A1A", marginBottom: "2rem" }}>Real experience, real capital</h2>
      ...
      <section style={{ padding: "7rem 2.5rem", background: "#FAFAF8" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <FadeIn>
            <p style={{ fontFamily: "system-ui, sans-serif", fontSize: "0.78rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "#7B0000", marginBottom: "2rem" }}>Analyst Training</p>
          </FadeIn>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", gap: "8rem", alignItems: "start" }}>
            <FadeIn delay={100}>
              <h2 style={{ fontFamily: "Palatino Linotype, Palatino, Georgia, serif", fontSize: "2.2rem", fontWeight: 400, lineHeight: 1.25, color: "#1A1A1A", marginBottom: "1.5rem" }}>Training for first-year analysts</h2>
              <p style={{ fontFamily: "system-ui, sans-serif", fontSize: "1rem", color: "#4A4A4A", lineHeight: 1.85 }}>
            ...
            <div>
              {[
                ...
                <FadeIn key={item.week} delay={i * 80}>
                  <div style={{ display: "grid", gridTemplateColumns: "110px 1fr", gap: "1.5rem", padding: "1.75rem 0", borderBottom: i < arr.length - 1 ? "1px solid #E5E3DF" : "none" }}>
      ...
      <section style={{ padding: "7rem 2.5rem", background: "#FFFFFF" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <FadeIn delay={150}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "2.5rem", alignItems: "center", justifyContent: "flex-start" }}>
              {["bofa", "jefferies", "point72", "deutsche", "lazard", "evercore", "pipersandler", "truist"].map((firm) => (
                <img key={firm} src={`/placements/${firm}.png`} alt={firm} style={{ height: 32, objectFit: "contain", filter: "grayscale(100%)", opacity: 0.65 }} />
              ))}
            </div>
          </FadeIn>
        </div>
